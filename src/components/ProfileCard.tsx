...
const ProfileCard: React.FC<ProfileCardProps> = ({
  ...
  return (
    <div className="bg-[var(--color-bg-secondary)] rounded-lg p-6 flex flex-col items-center shadow-[0_24px_80px_var(--color-shadow)] w-full max-w-xs border border-[var(--color-border-primary)] max-h-fit sticky top-10">
      ...
      <h2 className="text-xl font-bold mb-1 bg-gradient-to-r from-[var(--color-accent-tertiary)] via-[var(--color-accent-secondary)] to-[var(--color-accent-primary)] text-transparent bg-clip-text hover:scale-105 transform transition-transform duration-200 ease-in-out">{name}</h2>
      ...
      <div className="w-full space-y-4 text-sm mt-2">
        <div className="flex items-center text-[var(--color-text-secondary)] group hover:text-[var(--color-text-primary)] transition-all duration-300 transform hover:translate-x-1">
          ...
          <span className="break-words group-hover:text-[var(--color-text-primary)]">{location}</span>
        </div>
        ...
        <div className="flex items-center text-[var(--color-text-secondary)] group hover:text-[var(--color-text-primary)] transition-all duration-300 transform hover:translate-x-1">
          ...
          <a href={`mailto:${email}`} className="break-words hover:text-[var(--color-text-primary)] transition-all duration-300">
            {email}
          </a>
        </div>
        ...
        <div className="flex items-center text-[var(--color-text-secondary)] group hover:text-[var(--color-text-primary)] transition-all duration-300 transform hover:translate-x-1">
          ...
          <a href={`tel:${phone}`} className="break-words hover:text-[var(--color-text-primary)] transition-all duration-300">
            {phone}
          </a>
        </div>
      </div>
